<div style="text-align: center;font-size: 40px;">Linux container</div> 

## docker

- [rabbitmq](docker/rabbitmq.md) 
- [nginx](docker/nginx.md) 
- [jre8 for alpine](jre8/jre84alpine.md)
- [jre7 for alpine](jre7/jre74alpine.md)
- [nginx for react & vue](nginx/nginx4js.md)
- [Springboot Layering Docker Images](../../java/spring/boot/layer.md)

## k8s

- [kubic 安装 k8s](k8s/kubic.md)
- [kubectl 用法](k8s/kubectl.md)
- [Dashboard UI 部署](k8s/dashbord-ui.md)
- [RabbitMQ k8s 部署 脚本](k8s/rabbitmq.md)
- [mysql k8s 部署 脚本](k8s/mysql.yaml)

## registry

- [Docker registry 部署 与 自签证书](registry/registry.md)